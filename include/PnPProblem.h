...
public:
    ...
    void estimatePoseRANSAC( const std::vector<cv::Point3f> &list_points3d, const std::vector<cv::Point2f> &list_points2d, const cv::Mat& K, const cv::Mat& rvec, const cv::Mat& tvec, int flags, cv::Mat &inliers, int iterationsCount, float reprojectionError, double confidence, bool useExtrinsicGuess = false);
